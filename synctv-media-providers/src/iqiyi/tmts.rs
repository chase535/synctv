...
pub(super) async fn discover_tmts_media(
    ...
    .await
    {
        ...
        Err(_) => Err(ProviderClientError::Network(format!(
            "iQiyi HTTP resolver timed out after {}s",
...
async fn discover_tmts_media_inner(
    ...
    let request_url = build_tmts_url("https", &tvid, vid)?;
    let (response, transport) = match session.get_text(request_url.as_str()).await {
        Ok(response) => (response, "https"),
        Err(https_error) => {
            let fallback_url = build_tmts_url("http", &tvid, vid)?;
            match session.get_text(fallback_url.as_str()).await {
        ...
        Value::Object(object) => {
            for (key, child) in object {
                if matches!(key.to_ascii_lowercase().as_str(), "vid" | "videoid" | "video_id") {
                    if let Some(candidate) = value_as_string(child).filter(|value| looks_like_vid(value)) {
...
fn build_tmts_url(scheme: &str, tvid: &str, vid: &str) -> Result<Url, ProviderClientError> {
    ...
    let mut url = Url::parse(&format!("{scheme}://cache.m.iqiyi.com/jp/tmts/{tvid}/{vid}/"))
        .map_err(|error| ProviderClientError::Parse(error.to_string()))?;
    url.query_pairs_mut()
...
mod tests {
    ...
    #[test]
    fn tmts_signature_matches_formula() {
        assert_eq!(
            md5_hex("1700000000000d5fb4bd9d50c4be6948c97edd7254b0e123456789"),
            "54436dd3fac8758220a397ec39c5fb92"
        );
    }
}
