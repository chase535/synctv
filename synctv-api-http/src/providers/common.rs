...
use crate::http::{error::map_api_error, AppResult};
...
fn source_provider_param(value: Option<&str>) -> Result<i32, super::super::AppError> {
    ...
    synctv_core::models::SourceProvider::from_str(canonical)
        .map(synctv_api_common::impls::source_provider::core_source_provider_to_proto)
...
pub(crate) fn provider_instance_name(
    ...
    synctv_api_common::providers::common::provider_instance_name_from_query(query)
        .map_err(crate::http::error::map_api_error)
}
...
pub(crate) fn provider_instance_name_from_request_field(
    ...
    synctv_api_common::providers::common::provider_instance_name_from_value(body_instance_name)
...
mod tests {
    ...
    async fn provider_common_api_list_backends_includes_local_default_and_enabled_remote_instances(
        ...
        let api = synctv_api_common::providers::ProviderCommonApiImpl::new_with_runtime(
            provider_instance_manager,
            Arc::new(test_user_service(&pool)?),
            Arc::new(audit_service),
            synctv_api_common::providers::ProviderCommonApiRuntime {
                providers_manager,
                request_executor: Arc::new(
                    synctv_api_common::test_support::local_request_executor(),
                ),
            },
        );
        let backends = api
