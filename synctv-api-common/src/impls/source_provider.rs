...
pub fn proto_source_provider_required(value: i32) -> Result<SourceProvider, ApiError> {
...
pub fn proto_source_provider_filter(value: i32) -> Result<Option<SourceProvider>, ApiError> {
...
pub fn proto_source_provider_vec(values: Vec<i32>) -> Result<Vec<SourceProvider>, ApiError> {
    values
        .into_iter()
...
pub const fn core_source_provider_to_proto(provider: SourceProvider) -> i32 {
    match provider {
...
pub fn core_source_provider_vec_to_proto(providers: &[SourceProvider]) -> Vec<i32> {
    providers
        .iter()
        .copied()
